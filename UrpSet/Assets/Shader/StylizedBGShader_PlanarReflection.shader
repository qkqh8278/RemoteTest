Shader "ADA/BG/StylizedBGShader_PlanarReflection"
{
    Properties
    {
        ...
        [NoScaleOffset]_PlanarReflectionTexture ("Planar Reflection Texture", 2D) = "black"{}
        _PlanarReflectionAmount ("Planar Reflection Amount", Range(0,1)) = 1
        _PlanarReflectionDistortion ("Planar Reflection Normal Strength", float) = 0
        //_PlanarReflectionFresnelThreshold("PlanarReflection Fresnel Threshold", Range(0,1)) = 0.7
        //_PlanarReflectionFresnelSmooth("PlanarReflection Fresnel Smooth", Range(0,0.5)) = 0.25
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile_local _ _USEEMISSIVE_ON



            #pragma multi_compile_local _ _USEPLANARREFLECTION_ON

            TEXTURE2D(_BaseMap);   SAMPLER(sampler_BaseMap);   
            TEXTURE2D(_BumpMap);   SAMPLER(sampler_BumpMap);  
            TEXTURE2D(_MaskTex);   SAMPLER(sampler_MaskTex); 
            TEXTURE2D(_EmissiveMap);   SAMPLER(sampler_EmissiveMap); 
            #if _USEPLANARREFLECTION_ON
            ...
            half _PlanarReflectionAmount, _PlanarReflectionDistortion;

            //float _PlanarReflectionFresnelThreshold, _PlanarReflectionFresnelSmooth;
            ...
            v2f vert (appdata v)
            {
                v2f o;
                    ...
                    o.sPos = ComputeScreenPos(o.positionCS);
                    o.sPos.z = -mul( UNITY_MATRIX_MV, v.vertex ).z ;//COMPUTE_EYEDEPTH(o.sPos.z);
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float4 planarReflectionTex = SAMPLE_TEXTURE2D(_PlanarReflectionTexture, sampler_PlanarReflectionTexture, (i.sPos.xy / i.sPos.w) + (NormalTS.xy * _PlanarReflectionDistortion) );
                //float fresnel = smoothstep(_PlanarReflectionFresnelThreshold - _PlanarReflectionFresnelSmooth, _PlanarReflectionFresnelThreshold + _PlanarReflectionFresnelSmooth, 1- ndotv);
                col.rgb += lerp(0,planarReflectionTex.rgb , _PlanarReflectionAmount);
                #endif
