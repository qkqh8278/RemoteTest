Shader "ADA/Character/StylizedJewelShader"
{
    Properties
    {
        ...
        [Space(20)]
        [Header(Normal Expand and Z_bias)]
        [Space(10)]
        _NormalExpandMask("Normal Expand Mask Texture", 2D) = "white" {}
        _NormalExpand("Normal Expand", Range(-0.02,0.02)) = 0
        [Space(10)]
        
        _ZOffset("Z Offset", Range(-0.02,0.02)) = 0
        ...
        [Space(10)]
        [NoScaleOffset] _MaskTex ("Mask Texture ( R:Metallic, G:Roughness, B:AO )", 2D) = "white" {}
        _Metallic ("Metallic", Range(0,1)) = 1
        _Roughness ("Roughness", Range(0,1)) = 1
        _AoStrength("AO Strength", Range(0, 1)) = 1.0
        [Toggle]_InvertG("Invert G Chanel", float) = 0
        ...
        _ReflProbeBlurAmount("Reflection Probe Blur Amount", Range(0,6)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_NormalExpandMask);   SAMPLER(sampler_NormalExpandMask);  
            ...
            v2f vert (appdata input)
            {
                ...
                VertexPositionInputs vertexInput = GetVertexPositionInputs(input.vertex.xyz + input.normalOS * _NormalExpand * normalExpandMask);
                ...
                output.positionCS = TransformWViewToHClip(vertexInput.positionVS + float3(0,0,_ZOffset));
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float4 emissionTex = SAMPLE_TEXTURE2D(_EmissionTex, sampler_EmissionTex, i.uv);
                float3 emissive =  pow( abs(emissionTex.a), _EmissionEdgeSmooth) * _EmissionIntensity * _EmissionColor.rgb * emissionTex.rgb;
        ...
        Pass
    {
        ...
        Tags{"LightMode" = "ShadowCaster"}
           ...
           TEXTURE2D(_NormalExpandMask);   SAMPLER(sampler_NormalExpandMask);  
            ...
            VertexOutput ShadowPassVertex(VertexInput v)
            {
                ...
                float3 positionWS = TransformObjectToWorld(v.vertex.xyz + v.normal.xyz * _NormalExpand * normalExpandMask);
                float3 normalWS   = TransformObjectToWorldNormal(v.normal.xyz);
                float3 positionVS = TransformWorldToView( ApplyShadowBias(positionWS, normalWS, _MainLightPosition.xyz)) + float3(0,0,_ZOffset);
                float4 positionCS = TransformWViewToHClip(positionVS);


              
                o.vertex = positionCS * float4(1,1,_ZOffset,1);
        ...
        Pass
        {
            ...
            VertexOutput vert(VertexInput v)
            {
                ...
                float3 positionWS = TransformObjectToWorld(v.vertex.xyz + v.normalOS * _NormalExpand * normalExpandMask);
                float3 positionVS = TransformWorldToView(positionWS) + float3(0,0,_ZOffset);
