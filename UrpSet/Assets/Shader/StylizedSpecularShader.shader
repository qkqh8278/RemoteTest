Shader "ADA/Character/StylizedSpecularShader"
{
    Properties
    {
        ...
        [Space(10)]
        [NoScaleOffset] _MaskTex ("Mask Texture ( R:Metallic, G:Glossiness, B:AO, A:Emission)", 2D) = "white" {}
        _SpecularTex("Specular Color Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 DirectBDRFCustom(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS, half3 specular)
            {
            ...
            #endif
                half3 color = specularTerm * brdfData.specular * specular * max(0,_SpecularIntensity) + brdfData.diffuse; //LinearStep( _SpecularThreshold - _SpecularSmooth, _SpecularThreshold + _SpecularSmooth, specularTerm  ) * brdfData.specular * max(0,_SpecularIntensity) + brdfData.diffuse;
                return color;
            ...
            half3 LightingPhysicallyBasedCustom(BRDFData brdfData, half3 radiance, half3 lightDirectionWS, half3 normalWS, half3 viewDirectionWS, half3 positionWS, half3 specular)
            {             
            
                return DirectBDRFCustom(brdfData, normalWS, lightDirectionWS, viewDirectionWS, specular) * radiance;
            }

            half3 LightingPhysicallyBasedCustom(BRDFData brdfData, half3 radiance, Light light, half3 normalWS, half3 viewDirectionWS, half3 positionWS, half3 specular)
            {
                return LightingPhysicallyBasedCustom(brdfData, radiance, light.direction, normalWS, viewDirectionWS, positionWS , specular);
            }

            half3 LightingSpecularCustom(half3 lightColor, half3 lightDir, half3 normal, half3 viewDir, half3 specular, half glossiness)
            {
                float3 halfVec = SafeNormalize(float3(lightDir) + float3(viewDir));
                half NdotH = saturate(dot(normal, halfVec));
                half modifier = pow(NdotH, 1 / (glossiness + 0.0001) );
            ...
            half4 UniversalFragmentPBRCustom(InputData inputData, half3 albedo, half metallic, half3 specular, half smoothness, half occlusion, half3 emission, half alpha, half3 specularColor)
            {
                BRDFData brdfData;
                ...
                color += LightingPhysicallyBasedCustom(brdfData, radiance, mainLight, inputData.normalWS, inputData.viewDirectionWS, inputData.positionWS, specularColor);
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float glossiness = mask.g * _Glossiness;
                float occlusion =  lerp( 1 , mask.b , _AoStrength);
                //float specularMask = mask.r * _SpecularIntensity;

                float3 specularColor = SAMPLE_TEXTURE2D(_SpecularTex, sampler_SpecularTex, i.uv).rgb * _SpecularIntensity;
                ...
                #else
                    col = UniversalFragmentPBRCustom(inputData, albedo.rgb, metallic, 0.5 ,  glossiness , occlusion, emissive.rgb, albedo.a, specularColor);
                #endif
